...
!(function() {
    ...
    p = {
      ...
      loadAsync: function(a, b, c, d) {
        var e,
          ...
          h = 'n4',
          i = function() {},
          j = document.scripts[0];
        window.WebFontConfig || (window.WebFontConfig = {}),
          (g = window.WebFontConfig),
          g.custom || (g.custom = {}),
          g.custom.families || (g.custom.families = []),
          g.custom.urls || (g.custom.urls = []),
          g.custom.testStrings || (g.custom.testStrings = {}),
          g.custom.families.push(b),
          g.custom.urls.push(a),
          (g.custom.testStrings[b] = c),
          (e = g.fontactive || i),
          (g.fontactive = function(a, c) {
  ...
  if (
    (window.FontAwesomeCdnConfig &&
      ...
      (j = 'fa'),
      (k = 'ï‰€'),
      (l = e.webFontLoaderVersion),
      e.autoA11y.enabled && (a(d.bind(d, 'fa')), c(d.bind(d, 'fa'))),
      e.reporting.enabled &&
        b(e.reporting.domains, location.host) &&
        q.load(g + '/js/stats.js'),
      (n = 'https://' + f + '/' + h + '.css'),
      (m = 'https://' + f + '/webfontloader/' + l + '/webfontloader.js'),
      e.asyncLoading.enabled ? p.loadAsync(n, i, k, m) : p.load(n)),
    window.FortAwesomeConfig)
  ) {
    var r;
    (e = window.FortAwesomeConfig),
      (f = e.useUrl),
      (h = e.code),
      (i = e.familyName),
      (j = e.prefix),
      (k = e.testString),
      (l = e.webFontLoaderVersion),
      e.autoA11y.enabled && (a(d.bind(d, j)), c(d.bind(d, j))),
      (r = o.isIE(8, 'lte') ? h + '-sd' : h),
      (n = 'https://' + f + '/' + r + '.css'),
      (m =
        'https://use.fonticons.com/webfontloader/' + l + '/webfontloader.js'),
      e.asyncLoading.enabled ? p.loadAsync(n, i, k, m) : p.load(n);
  }
})();
